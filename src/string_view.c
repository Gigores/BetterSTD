...
btr_string_view_s BTR_StringView_cropLeft(btr_string_view_s string, unsigned int charCount)
{
...
btr_string_view_s BTR_StringView_cropRight(btr_string_view_s string, unsigned int charCount)
{
    for (unsigned int i = 0; i < charCount; i++)
    {
        size_t offset = 0;
...
btr_string_view_s BTR_StringView_revertLeft(btr_string_view_s string, unsigned int charCount)
{
    for (unsigned int i = 0; i < charCount; i++)
    {
        ...
        if (charSize == 0) return string;
        string.data   -= charSize;
        string.length += charSize;
    }
    return string;
}
btr_string_view_s BTR_StringView_revertRight(btr_string_view_s string, unsigned int charCount)
{
    for (unsigned int i = 0; i < charCount; i++)
    {
...
btr_string_view_s BTR_StringView_trimLeft(btr_string_view_s string)
{
    while (isspace(*(unsigned char *)(string.data)))
        string = BTR_StringView_cropLeft(string, 1);
    return string;
}
btr_string_view_s BTR_StringView_trimRight(btr_string_view_s string)
{
    while (string.length > 0 && isspace(*(unsigned char *)(string.data + string.length - 1)))
        string = BTR_StringView_cropRight(string, 1);
    return string;
}
btr_string_view_s BTR_StringView_trim(btr_string_view_s string)
{
    string = BTR_StringView_trimLeft(string);
    string = BTR_StringView_trimRight(string);
    return string;
}
